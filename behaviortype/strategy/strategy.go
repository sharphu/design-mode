/*
概念与定义：策略模式（Strategy Pattern）是一种行为设计模式，允许定义一系列算法，将每种算法封装到独立的策略类中，并使这些策略类可以互相替换。
这样可以让客户端代码在运行时选择具体的算法。
例子：用一个支付系统来演示策略模式。不同的支付方式（如信用卡支付和 PayPal 支付）都是独立的策略，客户端可以动态选择其中一种支付方式。
*/
